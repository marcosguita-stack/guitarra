...
**Purpose:** Party Mode MUST remain active even after Claude Code compacts prior messages. Context compaction discards old conversation content but this workflow must persist.

### Compaction State Block

After EVERY agent response round, mentally maintain this compact state block. If you detect that context was compacted (conversation history feels truncated or you lost track of prior discussion), immediately reconstruct party mode from this protocol:

```yaml
# PARTY-MODE ACTIVE STATE (survive compaction)
party_active: true
workflow: party-mode
agents_source: "{project-root}/{bmad_folder}/_cfg/agent-manifest.csv"
workflow_file: "{project-root}/{bmad_folder}/core/workflows/party-mode/workflow.md"
step: 2  # discussion-orchestration (always 2 while party is active)
exit_triggers: ['*exit', 'goodbye', 'end party', 'quit']
```

### Recovery After Compaction

If you detect compaction occurred (loss of conversation history):

1. **DO NOT exit party mode** — party mode is still active
2. **DO NOT re-introduce agents** — skip the welcome, they are already here
3. **Re-read the agent manifest** at `{project-root}/{bmad_folder}/_cfg/agent-manifest.csv` to refresh agent personalities
4. **Acknowledge naturally** — a brief "Continuando..." or seamlessly continue the discussion
5. **Resume Step 2** (Discussion Orchestration) immediately
6. **Maintain agent selection intelligence** — continue rotating and selecting relevant agents

### Compaction Detection Signals

You are likely post-compaction if:
- You see `<system-reminder>` about compressed messages
- Prior conversation rounds are missing from your visible history
- You feel loss of context about what was being discussed

### Non-Negotiable Rule

**Party Mode is PERSISTENT until the user explicitly triggers an exit condition.** No system event, context compaction, or internal process may deactivate party mode. Only user-initiated exit triggers (`*exit`, `goodbye`, `end party`, `quit`) end the party.
